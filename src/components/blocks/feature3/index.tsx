                  ...
                  >
                    ...
                    {item.image && (
                      <div className="mt-6 block border bg-muted/50 px-4 py-6 lg:hidden">
                        <div className="aspect-square w-96 mx-auto">
                          <img
                            src={item.image?.src}
                            alt={item.image?.alt || item.title}
                            className="h-full w-full rounded-md border object-contain shadow-sm"
                            loading="lazy"
                            decoding="async"
                          />
                        </div>
              ...
              {section.items?.map((item, index) => {
                ...
                return (
                  <TabsContent
                    key={index}
                    value={`tab-${index + 1}`}
                    className="aspect-video mx-auto max-w-6xl"
                  >
                    {item.image && (
                      <img
                        src={item.image.src}
                        alt={item.image.alt || item.title}
                        className="h-full w-auto mx-auto rounded-xl border object-contain shadow-sm"
                        loading="lazy"
                        decoding="async"
                      />
                    )}
